...
export default class Db {
    ...
    async findAllManagers() {
        return this.query("SELECT * FROM employee WHERE manager_id IS NULL;");
    }
    // TODO: Get all roles with the job title, role id, the department that role belongs to, and the salary for that role
    async findAllRoles() {
        return this.query("SELECT role.id, role.title, role.salary, department.name AS department FROM role LEFT JOIN department ON role.department_id = department.id;");
    }
